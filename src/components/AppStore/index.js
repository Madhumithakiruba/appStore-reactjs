...
class AppStore extends Component {
  state = {searchInput: '', activeItemId: tabsList[0].tabId}
  ...
  getAppDetails = searchResults => {
    const {activeItemId} = this.state
    const filterAppDetails = searchResults.filter(
      eachApp => eachApp.category === activeItemId,
    )
    return filterAppDetails
  }
  ...
  render() {
    ...
    const filterAppDetails = this.getAppDetails(searchResults)
          ...
          <ul className="tabitem-list">
            {tabsList.map(eachTabItem => (
              <TabItem
                ...
                isActive={activeItemId === eachTabItem.tabId}
              />
            ))}
          </ul>
          <ul className="appitem-list">
            {filterAppDetails.map(eachAppItem => (
